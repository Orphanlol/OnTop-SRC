...
// *://login.i-ready.com/*
// gets all units
fetch("https://cdn.i-ready.com/instruction/phoenix/master/121/lessonmap.json").then(response => response.json())

// loads units
.then(unitData => units = unitData).then(function () {
  ...
  function listLessons(i = 0, j = 0) {
    fetch(`https://cdn.i-ready.com/instruction/phoenix-content/${Object.keys(units)[i].includes("ELA") ? "reading" : "math"}/${Object.keys(units[Object.keys(units)[i]])[j]}/lessonStructure.json`).then(response => response.json()).then(function (lessonData) {
      ...
      for (var k = 0; k < lessons[Object.keys(units[Object.keys(units)[i]])[j]].lessonStructureObj.slideData.length; k++) {
        var slide = {
          type: "unknown",
          name: lessons[Object.keys(units[Object.keys(units)[i]])[j]].lessonStructureObj.slideData[k].name
        };
...
lessonSelect.onchange = function () {
  ...
  for (var i = 0; i < lessons[lessonSelect.value].slides.length; i++) {
    ...
    if (slide.type != "unknown") {
      data.innerHTML += `<br><i>"${slide.src.split("/")[slide.src.split("/").length - 1]}"</i> <br> <video controls width="250" src="${slide.src}"></video> <br>`;
    }
  }
};
